...
TEST_IMG_FOLDER=${TEST_IMG_FOLDER:-${VAL_IMG_FOLDER:-$DATA_ROOT}}
TEST_ANN_FILE=${TEST_ANN_FILE:-${VAL_ANN_FILE:-$DATA_ROOT/annotations/val_stride6.json}}
...
VAL_TOTAL_BATCH_SIZE=${VAL_TOTAL_BATCH_SIZE:-12}
VAL_NUM_WORKERS=${VAL_NUM_WORKERS:-8}
MAX_DETS=${MAX_DETS:-500}
OUTPUT_DIR=${OUTPUT_DIR:-./output/acds_dfine_hgnetv2_s_uavdt}
CHECKPOINT=/data/libaichuan/Projects/SOD/D-FINE/output/acds_dfine_hgnetv2_s_uavdt/best_stg1.pth
...
if [[ "$REMAP_CATEGORY_IDS" == "1" || "$REMAP_CATEGORY_IDS" == "true" ]]; then
  TEST_ANN_FOR_DFINE="$DFINE_ANN_DIR/val_stride6_zero_based.json"
